...
<!--
各位同学上午好，很开心能在福州与你们相遇，也很开心收到主办方邀请来做这次分享。这里有个问题想问下同学们，兴生力培训好不好玩，培训课程累不累，那好了，今天的课程内容非常轻松，主打一个陪伴，内容不多，很轻松的
-->
...
<!--
首先简单做个自我介绍，我叫做余自立，任职于技术服务中心一名前端开发工程师，主要负责兴智汇基础平台底座JS-SDK项目，相关项目有兴智汇底座JS-SDK、兴智汇通用组件（如云相册、通用表单、统一待办等组件）。

这里简单讲解下云相册组件，后续涉及到业务场景需要外采图片、视频的应用场景可以集成使用，如移动端拍摄图片后如何快速上传到公共内网并关联该笔业务的流水号，操作链路为：相机硬件拍摄、客户端本地存放、分片上传到兴智汇通用文件、（如有单独申请icmp）同步备份到指定系统申请的icmp目录、拿到icmp影像批次号后保存，后续文件可以通过该链路再下载，实现素材多端使用，调用细节可以参考行内开源系统xzh-jssdk项目的调用示例，当然也可以随时联系我，后续对上述组件有兴趣的小伙伴可以一起交流下。

课前看了下本次参加的名单，应该有不少的小伙伴已经线下一起面基过了，期待更多的小伙伴来一起交流成长。

接下来我们一起进入前端技术概论与基础技术这门课程的学习内容。
...
可能有不少非前端技术领域的同学，默认为前端仅是根据设计稿做一些页面、页面切换、用户交互反馈的等功能，俗称“页面仔”，那是不是我们前端同学仅仅只能做页面呢？
...
举个栗子，计财部业务老师想要一个数据驾驶舱功能，按传统的前端开发方案，编写页面结构、样式、脚本、接口联调（在完全没有存量组件库支持的情况下，一个前端工程师需要2-3天提供演示demo）,但在现有的前端开发环境下小半天足矣（甚至还可以完成公共组件的单元测试），类似的构建工具、包管理、可视化组件、mock-server等工具可大幅度降低demo研发时长以及扩展功能领域。在固定业务场景类型的情况还可以将页面组件进行拆分，以便其他场景快速复用搭建页面。甚至接入copilot、AWS等AI辅助编码工具，让编码过程极度丝滑。

课后也推荐对前端有兴趣的同学，可以尝试下入门教程，前端是相对比较好上手的，“即编即所得”的特性让初学者在上手练习时会很有学习成就感。但随着学习阶梯的曲线陡峭度变化，在进阶阶段会涉猎到如AST、GPU、模型、算法等偏难的知识内容，相信在座的前端同学应该是有体验的。

[click]接下来呢，我们将进入前端的基础知识内容的第一部分，超文本标记语言HTML。
...
术语 HTML5 本质上是指一组现代 Web 技术的流行词汇。这包括 HTML 动态标准，以及用于增强存储、多媒体和硬件访问的API。
...
[click] 音视频 video、audio，常用混迹于B站的小伙伴应该非常熟悉HLS（fmp4），该协议全称 HTTP Live Streaming，由苹果公司提出的一种基于HTTP的流媒体网络传输协议
[click] 矢量图形能力支持 canvas svg （邀请现场的设计同学讲解下位图与矢量图的区别）。可视化主流工具库多是通过此API封装完成，如echarts图表库，知名网站stateofjs的数据图表同样是通过canvas绘制而成，后续章节中我们会展示该图表效果
[click] 还有如本地存储 localStorage | sessionStorage | indexedDB 前两个API相信同学们非常熟悉，在此就不做赘述了。indexedDB是一种底层 API，用于在客户端存储大量的结构化数据（也包括文件/二进制大型对象（blobs）），且此特性在webworker可用
 webworker 为 Web 内容在后台线程中运行脚本提供了一种简单的方法。线程可以执行任务而不干扰用户界面,所以在worker中是无法操作dom元素的
 地理位置API Geolocation

接下来我们快速进入基础知识的第二部分CSS的世界
-->
...
当然原生css在编写上的效率以及复杂场景的复用性受限于语法特性，所以我们可以引入一些样式预处理器、如less、sass、postcss等工具库，支持变量、混合、嵌套、运算、转义、函数、作用域等功能特性;或选择如css原子化工具如tailwind、unocss等工具辅助我们开发，提升编码效率的同时提高代码复用度，非常适合喜好使用预设、快捷键位的同学。

当然如果还未尝试过以上工具的同学也可以联系我获取一些上线项目代码示例。
...
当同一个元素有多个规则集声明时，分配给CSS声明的一个权重，由匹配的选择器类型的权重数值决定。其中有团队协作项目的前端同学、或改造第三方组件库的同学对于这个权重计算非常有经验了。
...
> 备注：通用选择器（*）、组合符（+、>、~）和调整优先级的选择器 :where 不会影响优先级。以及否定(:not)和任意匹配(:is())伪类本身对优先级没有影响，但他们的参数则会带来影响
...
常见的可以继承的属性有font-family 字体 font-size 字号 font-color 字体颜色、line-height 字高等字体属性、文字展示属性
...
<!--
我们最熟悉的微信朋友圈的九宫格图片布局就可以使用3行3列的网格实现布局。
...
接下来我们来看下tailwindcss和unocss两个示例demo：
-->
...
UnoCSS 通过非常高效的字符串拼接来直接生成对应的 CSS 而非引入整个编译过程（该库内置了很多实用的正则表达式，有兴趣的同学可以去参考下）。同时，UnoCSS 对类名和生成的 CSS 字符串进行了缓存，当再次遇到相同的实用工具类时，它可以绕过整个匹配和生成的过程。

上述是unocss的一些基本配置，以及动态拼配规则，辅助开发者快速生成样式代码。
...
[click]Generator 函数,ES6 引入的特殊函数，用于生成迭代器对象。可以通过 yield 暂停和恢复执行。常用于异步操作和控制流管理
...
[click]4. 事件驱动 (Event Loop),JavaScript 异步的底层机制依赖事件循环（Event Loop）。示例任务执行顺序为：同步任务、微任务、宏任务
...
- 生命周期,简要分为三个阶段，1、组件挂载；2、组件状态更新；3、组件卸载
...
- 生态周边，ssr Next.js、 ahooks工具库、antd 组件库等
...
<!--
从图表中我们可以看出，2024年webpack和vite在用户使用量上高居榜首，且随着vite的版本更新、完善配置体系、极佳的HMR热更新体验，逐步呈现超越webpack的趋势，得益于依赖和源码的模块区分，基于go语言开发的esbuild以及原生ESM的加持下，实现开发服务器秒级启动和无感热更新，出道便迅速俘获众多开发者的心
